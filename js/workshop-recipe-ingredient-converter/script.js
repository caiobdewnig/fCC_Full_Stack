...
const updateResultsList = (e) => {
  e.preventDefault();
  resultList.innerHTML = "";
  for (let i = 0; i < units.length; i++) {
    if (units[i] !== unitToConvert.value) {
      const conversionRate = (conversionTable[unitToConvert.value][units[i]] * ingredientQuantity.value).toFixed(2);
      resultList.innerHTML += `<li>${ingredientName.value}: ${conversionRate} ${units[i]}</li>`;
    }
  }
};
